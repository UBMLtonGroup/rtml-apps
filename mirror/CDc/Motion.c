...
int MOTION_findIntersection(Motion * one, Motion * two, Vector3d * v) {
    benchmarkRapita(rapita_set(RAPITA_FIND_INTERSECTION));
    ...
    if (a != 0.0f) {
        ...
        if (discr < 0.0f) {
            benchmarkRapita(rapita_set(RAPITA_DONE+RAPITA_FIND_INTERSECTION));
            return 0;
        }
        ...
        if (v1 <= v2 && (v1 <= 1.0f && 1.0f <= v2 || v1 <= 0.0f && 0.0f <= v2 || 0.0f <= v1 && v2 <= 1.0f)) {
            ...
            if (z1col > MIN_Z && z1col <= MAX_Z && x1col >= MIN_X
                ...
                VECTOR3D_init2(v, x1col, y1col, z1col);
                benchmarkRapita(rapita_set(RAPITA_DONE+RAPITA_FIND_INTERSECTION));
                return 1;
            }
        }
        ...
        if (dist <= r) {
            VECTOR3D_init2(v, one->pos_one->x, one->pos_one->y, one->pos_one->z);
            benchmarkRapita(rapita_set(RAPITA_DONE+RAPITA_FIND_INTERSECTION));
            return 1;
        }
    }
...
char * MOTION_toString(char linetoprint[], Motion * mt) {
    char buf[30], buf1[30], buf2[30];
    char * acStr = AIRCRAFT_toString(buf1, mt->aircraft);
    char * pos1Str = VECTOR3D_toString(buf, mt->pos_one);
    char * pos2Str = VECTOR3D_toString(buf2, mt->pos_two);
    sprintf(linetoprint, "MOTION of %s from %s to %s", acStr, pos1Str, pos2Str);
    return linetoprint;
}
